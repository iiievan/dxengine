...
public:
    ...
    static IndexedTriangleList<V> MakeTesselated(int long_div)
    {
        ...
        const float longitude_angle = 2.0f * PI / float(long_div);

        std::vector<V> vertices;
        for (int iLong = 0; iLong < long_div; iLong++)
        {
            vertices.emplace_back();
            auto v = dx::XMVector3Transform(base, dx::XMMatrixRotationZ(longitude_angle * (float)iLong));
        ...
        for( unsigned short iLong = 0; iLong < long_div; iLong++ )
        {
            ...
            indices.push_back( (iLong + 1) % long_div );
