    ...
    : TestObject(gfx, rng, adist, ddist, odist, rdist)
{
    ...
    {
        auto pvs = std::make_unique<VertexShader>(gfx, L"shaders/BlendedPhong.vs.cso");
        auto pvsbc = pvs->GetBytecode(); // Blob for InputLayout
        AddStaticBind(std::move(pvs));

        AddStaticBind(std::make_unique<PixelShader>(gfx, L"shaders/BlendedPhong.ps.cso"));
